...
public final class Party_GUI extends Util_GUI {
    public void PartyGUI_Main(Player player) {
        ...
        if (Main_ServerOption.PartyJoiner.containsKey(player) == false) {
            ...
            if (Main_ServerOption.Party.get(Main_ServerOption.PartyJoiner.get(player)).getLeader().equalsIgnoreCase(player.getName()) == true) {
                ...
                if (Main_ServerOption.Party.get(Main_ServerOption.PartyJoiner.get(player)).getLock() == false)
    ...
    public void PartyListGUI(Player player, short page) {
        ...
        if (Main_ServerOption.Party.size() <= 0) {
            ...
            byte loc = 0;
            for (int count = page * 45; count < Main_ServerOption.Party.size(); count++) {
                if (count > Main_ServerOption.Party.size() || loc >= 45) break;
                if (Main_ServerOption.Party.get(a[count]).getLock() == false)
                    Stack2(ChatColor.WHITE + "" + ChatColor.BOLD + Main_ServerOption.Party.get(a[count]).getTitle(), 54, 0, 1, Arrays.asList(ChatColor.GRAY + "파티 생성 시각 : " + ChatColor.DARK_GRAY + a[count], "", ChatColor.AQUA + "파티장 : " + ChatColor.GRAY + Main_ServerOption.Party.get(a[count]).getLeader(), ChatColor.AQUA + "인원 : " + ChatColor.GRAY + Main_ServerOption.Party.get(a[count]).getPartyMembers() + "/" + Main_ServerOption.Party.get(a[count]).getCapacity()), loc, inv);
                else
                    Stack2(ChatColor.WHITE + "" + ChatColor.BOLD + Main_ServerOption.Party.get(a[count]).getTitle(), 130, 0, 1, Arrays.asList(ChatColor.GRAY + "파티 생성 시각 : " + ChatColor.DARK_GRAY + a[count], "", ChatColor.AQUA + "파티장 : " + ChatColor.GRAY + Main_ServerOption.Party.get(a[count]).getLeader(), ChatColor.AQUA + "인원 : " + ChatColor.GRAY + Main_ServerOption.Party.get(a[count]).getPartyMembers() + "/" + Main_ServerOption.Party.get(a[count]).getCapacity(), "", ChatColor.RED + "[잠김]", ChatColor.RED + "파티 가입을 하시려면", ChatColor.RED + "파티장에게 문의하세요!"), loc, inv);
                loc++;
            }
        }
    ...
    public void PartyMemberInformationGUI(Player player, short page, long PartyCreateTime, boolean isLeaderChange) {
        ...
        Player[] Member = Main_ServerOption.Party.get(PartyCreateTime).getMember();
        byte loc = 0;
        for (int count = page * 45; count < Member.length; count++) {
            ItemStack ph = null;
            if (count > Member.length || loc >= 45) break;
            Damageable pl = Member[count];
            if (player.getName().equals(Main_ServerOption.Party.get(PartyCreateTime).getLeader())) {
                if (isLeaderChange == false) {
                    if (!Member[count].getName().equals(Main_ServerOption.Party.get(PartyCreateTime).getLeader()))
        ...
        if (slot == 44)//나가기
        {
            ...
            else if (slot == 10)//파티 개설 / 파티 정보
            {
                if (ChatColor.stripColor(event.getCurrentItem().getItemMeta().getDisplayName()).compareTo("파티 개설") == 0) {
                    ...
                    PartyMemberInformationGUI(player, (short) 0, Main_ServerOption.PartyJoiner.get(player), false);
            ...
            else if (slot == 14)//파티 탈퇴
            {
                ...
                PartyMemberInformationGUI(player, (short) 0, Main_ServerOption.PartyJoiner.get(player), true);
    ...
    public void PartyMemberInformationGUIClick(InventoryClickEvent event) {
        ...
        if (slot == 53)//닫기
        {
            ...
            if (slot == 45)//이전 목록
                ...
                PartyMemberInformationGUI(player, (short) (Integer.parseInt(event.getInventory().getTitle().split(" : ")[1]) - 2), Main_ServerOption.PartyJoiner.get(player), isLeaderChange);
            else if (slot == 50)//다음 페이지
                PartyMemberInformationGUI(player, (short) Integer.parseInt(event.getInventory().getTitle().split(" : ")[1]), Main_ServerOption.PartyJoiner.get(player), isLeaderChange);
            else {
                if (event.isLeftClick()) {
                    ...
                    if (isLeaderChange == false) {
                        Main_ServerOption.Party.get(Main_ServerOption.PartyJoiner.get(player)).KickPartyMember(player, ChatColor.stripColor(event.getCurrentItem().getItemMeta().getDisplayName()));
                        PartyMemberInformationGUI(player, (short) (Integer.parseInt(event.getInventory().getTitle().split(" : ")[1]) - 1), Main_ServerOption.PartyJoiner.get(player), isLeaderChange);
                    }
                }
            }
        }
