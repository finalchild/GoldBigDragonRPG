...
public class Dungeon_ServerTask {
                ...
                switch (GridImage) {
                    ...
                    case '◇':
                        ...
                        if (GridImage == '◇') {
                            ...
                            for (char c = 0; c < 4; c++)
                                randomchest[c] = 146;
                            byte randomnum = (byte) new Util_Number().RandomNum(0, 3);
    ...
    public void CreateKeyRoom(Dungeon_ScheduleObject DSO) {
        ...
        switch (KeyGridImage) {
            ...
            case '▷':
            case '♥':
                byte color = (byte) new Util_Number().RandomNum(1, 5);
                switch (color) {
                    case 1:
                        color = 15;
                        break;
            ...
            for (int count2 = 0; count2 < DSO.getDungeonMakerSize(); count2++) {
                if (Bukkit.getPlayer(DSO.getDungeonMaker(count2)) != null) {
                    ...
                    if (Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isBgmOn()) {
                        if (player.getLocation().getWorld().getName().compareTo("Dungeon") == 0) {
                            new io.github.goldbigdragon.goldbigdragonrpg.dependency.NoteBlockAPIMain().Stop(Bukkit.getPlayer(DSO.getDungeonMaker(count2)));
                            new io.github.goldbigdragon.goldbigdragonrpg.dependency.NoteBlockAPIMain().Play(Bukkit.getPlayer(DSO.getDungeonMaker(count2)), SoundTrack);
                        }
                    }
                }
            }
