...
public class ETC {
    ...
    public void UpdatePlayerHPMP(Player player) {
        ...
        int MaxHealth = Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getStat_MaxHP() + BonusHealth;
        if (MaxHealth > 0)
    ...
    public void SlotChangedUpdatePlayerHPMP(Player player, ItemStack newSlot) {
        ...
        int BonusHealth = new Battle_Calculator().getPlayerEquipmentStat(player, "생명력", false, newSlot)[0];
        int MaxHealth = Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getStat_MaxHP() + BonusHealth;
