...
public class Party_Object
{
	...
	public Party_Object(Long CreateTime, Player player, String PartyName)
	{
		...
		GBD_RPG.Main_Main.Main_ServerOption.PartyJoiner.put(player, CreateTime);
		GBD_RPG.Main_Main.Main_ServerOption.Party.put(CreateTime, this);
	}
						...
						for(byte count = 0; count < this.PartyCapacity; count++)
							if(this.PartyMember[count] == null)
							{
								if(player.isOnline())
								{
									...
									new GBD_RPG.Effect.Effect_Sound().SP(player, Sound.BLOCK_WOODEN_DOOR_OPEN, 1.1F, 1.0F);
								}
								this.PartyMember[count] = player.getName();
								GBD_RPG.Main_Main.Main_ServerOption.PartyJoiner.put(player, this.CreateTime);
	...
	{
		GBD_RPG.Main_Main.Main_ServerOption.PartyJoiner.remove(player);
		if(player.isOnline())
		{
			player.sendMessage(ChatColor.RED + "[파티] : 파티를 탈퇴하였습니다!");
			new GBD_RPG.Effect.Effect_Sound().SP(player, Sound.BLOCK_WOODEN_DOOR_CLOSE, 1.1F, 1.0F);
		}
		if(getPartyMembers() == 1)
		{
			GBD_RPG.Main_Main.Main_ServerOption.Party.remove(this.CreateTime);
			return;
		}
		for(byte count = 0; count < this.PartyCapacity; count++)
