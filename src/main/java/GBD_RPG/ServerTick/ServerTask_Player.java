...
public class ServerTask_Player
{
	...
	public void Gamble_SlotMachine_Rolling(long UTC)
	{
		...
		if(count<STSO.getMaxCount())
		{
			...
			new GBD_RPG.Admin.Gamble_GUI().SlotMachine_RollingGUI(STSO.getString((byte)0), ItemID, false, STSO.getString((byte)1));
		...
		else
		{
			...
			new GBD_RPG.Admin.Gamble_GUI().SlotMachine_RollingGUI(STSO.getString((byte)0), ID , true, STSO.getString((byte)1));
			...
			if(Bukkit.getServer().getPlayer(STSO.getString((byte)0)) != null)
			{
				if(Bukkit.getServer().getPlayer(STSO.getString((byte)0)).isOnline()==true)
				{
					...
					{
						new GBD_RPG.Effect.Effect_Sound().SP(player, Sound.ENTITY_IRONGOLEM_HURT, 0.8F, 0.9F);
