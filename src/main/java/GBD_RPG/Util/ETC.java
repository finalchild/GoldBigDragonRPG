...
public class ETC {
    ...
    public void UpdatePlayerHPMP(Player player) {
        if (Bukkit.getPluginManager().isPluginEnabled("MagicSpells") == true
                && GBD_RPG.Main_Main.Main_ServerOption.MagicSpellsCatched == true) {
        ...
        int MaxHealth = GBD_RPG.Main_Main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getStat_MaxHP() + BonusHealth;
        if (MaxHealth > 0)
    ...
    public void SlotChangedUpdatePlayerHPMP(Player player, ItemStack newSlot) {
        ...
        int BonusHealth = new GBD_RPG.Battle.Battle_Calculator().getPlayerEquipmentStat(player, "생명력", false, newSlot)[0];
        int MaxHealth = GBD_RPG.Main_Main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getStat_MaxHP() + BonusHealth;
